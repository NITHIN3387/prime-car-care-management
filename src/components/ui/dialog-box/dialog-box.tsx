...
export const DialogBox = forwardRef<HTMLElement, DialogBoxProps>(
  ({ className, children, open, setOpen, title, ...props }, ref) => {
    return (
      <div
        className={clsx(
          "fixed h-dvh w-full top-0 left-0 flex justify-center items-center backdrop-blur-sm z-10 p-4",
          !open && "hidden"
        )}
        id="blurBg"
        onClick={(e) =>
      ...
      >
        <article
          className={cn("p-6 bg-black rounded-lg space-y-8 animate-dialog-box-open", className)}
          ref={ref}
          {...props}
        >
          {title ? (
